...
class ContadorUsuarios {
    ...
    guardarEstadisticas(aciertos, errores) {
        if (!this.usuarioActual) {
            alert('Por favor, ingresa un nombre de usuario primero');
            return;
        }

        console.log("aciertos guardar ",aciertos);
        const estadisticas = {
            hisA: aciertos,
            hisE: errores,
    ...
    cargarUsuarioActual() {
		...
		userDisplay.textContent = `Usuario actual: ${this.usuarioActual.toUpperCase() || 'No seleccionado'}`;
    ...
    actualizarInterfaz() {
        ...
        // Calcular porcentaje
        const total = estadisticas.hisA + estadisticas.hisE;
        const porcentaje = total > 0 ? ((estadisticas.hisA / total) * 100).toFixed(1) : 0;
    ...
    mostrarListaUsuarios() {
        ...
        usuarios.forEach(usuario => {
            const total = usuario.hisA + usuario.hisE;
            const porcentaje = total > 0 ? ((usuario.hisA / total) * 100).toFixed(1) : 0;
            
            html += `
                <div style="border: 1px solid #ccc; padding: 10px; margin: 5px 0; border-radius: 5px;">
                    <strong>${usuario.nombre}</strong>
                    <div>Aciertos: ${usuario.hisA} | Errores: ${usuario.hisE}</div>
...
function registrarAcierto() {
    ...
    stats.hisA++;
    contador.guardarEstadisticas(stats.hisA, stats.hisE);
}
...
function reiniciarContador() {
    if (confirm('¿Estás seguro de que quieres reiniciar el contador?')) {
        contador.guardarEstadisticas(0, 0);
    }
}
